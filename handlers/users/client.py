...
from states.client import ClientForm, EditClient, DeleteCl, Info
from aiogram.dispatcher import FSMContext
from loader import dp
from keyboards.default.register import prava, text_client_reg, text_client_up, info_btn, text_info, usertype
...
@dp.message_handler(text=["Тарифлар", "Тарифы", "Тесты", "Тест бўйича маълумотлар", "⬅️Oртга", "⬅️Назад",
                          "Онлайн", "Информация об онлайн уроках"])
async def ind(mes: Message):
    if mes.text == "Тарифлар":
        ...
        await mes.answer(f"Войдите в этот бот для тестирования <b>{text_info()['bot_link']}</b>")
    elif mes.text == "Тест бўйича маълумотлар":
        await mes.answer(f"Тест учун шу ботга киринг <b>{text_info()['bot_link']}</b>")
    elif mes.text == "Онлайн":
        ...
        await mes.answer("Керакли бўлимни танланг 👇", reply_markup=usertype(lang))
    elif mes.text == "⬅️Назад":
        await mes.answer("Выберите нужный раздел 👇", reply_markup=usertype(lang))
